...
- `pluginctl build` pushes to the node-local registry at `NODE_CONTROL_PLANE_IP:5000`
...
## 6. [BUG][P2] H00F `lan0` down → node-local registry (`NODE_CONTROL_PLANE_IP:5000`) unreachable

**Likely scope:** node H00F hardware/network

### Problem
`pluginctl build` pushes the built image to the node-local WES registry at
`NODE_CONTROL_PLANE_IP:5000` so `pluginctl run` can pull it. On H00F this push fails.

### Evidence
```
pinging container registry NODE_CONTROL_PLANE_IP:5000: Get "https://NODE_CONTROL_PLANE_IP:5000/v2/":
  dial tcp NODE_CONTROL_PLANE_IP:5000: connect: connection refused
```
`ip addr` shows `lan0` (the `NODE_CONTROL_PLANE_IP/24` interface) as
...
node-local registry) back up. Note the same `NODE_CONTROL_PLANE_IP` VIP is used for the
...
- Definitive test on the node:
  ...
  `dial tcp NODE_CONTROL_PLANE_IP:6443: i/o timeout` (or `http2: client connection lost`).
- Journal: `http2: client connection lost` degrading to
  `dial tcp NODE_CONTROL_PLANE_IP:6443: i/o timeout`, failed node-lease updates, pods unable
