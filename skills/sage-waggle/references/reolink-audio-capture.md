...
curl -s "http://CAMERA_IP:HTTP_PORT/api.cgi?cmd=GetEnc&user=USER&password=CAMERA_PASSWORD" \
  -d '[{"cmd":"GetEnc","action":0,"param":{"channel":0}}]'

# Enable audio
curl -s "http://CAMERA_IP:HTTP_PORT/api.cgi?cmd=SetEnc&user=USER&password=CAMERA_PASSWORD" \
...
```bash
curl -s "http://CAMERA_IP:HTTP_PORT/api.cgi?cmd=GetNetPort&user=USER&password=CAMERA_PASSWORD" \
...
--camera 'http://CAMERA_IP:PORT/flv?port=1935&app=bcs&stream=channel0_sub.bcs&user=sage&password=CAMERA_PASSWORD'
```
