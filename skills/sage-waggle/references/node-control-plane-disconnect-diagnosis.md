...
```
ssh USER@node-<VSN>.sage 'hostname; uptime'
```
...
```
ssh USER@node-<VSN>.sage 'sudo kubectl get pods -n ses --request-timeout=30s'
```
...
- **`dial tcp NODE_CONTROL_PLANE_IP:6443: i/o timeout` (or `http2: client connection lost`)
  -> control plane UNREACHABLE from this node.** `NODE_CONTROL_PLANE_IP:6443` is the
...
```
ssh USER@node-<VSN>.sage 'timeout 8 bash -c "</dev/tcp/NODE_CONTROL_PLANE_IP/6443" && echo OPEN || echo UNREACHABLE'
```
...
ssh USER@node-<VSN>.sage '
  ...
  ip route get NODE_CONTROL_PLANE_IP               # route present? (via ... dev wan0)
...
`http2: client connection lost` degrading to `dial tcp NODE_CONTROL_PLANE_IP:6443:
...
```
ssh USER@node-<VSN>.sage 'sudo systemctl restart k3s'
```
...
- SES token file on Flint: `~/AI-projects/slack-hummingbird/secrets/sage-token.txt`
  (read with `read -r TOKEN < <path>`; Hermes mangles `$(cat ...)`).
- Node SSH: `ssh USER@node-<VSN>.sage` (uses `*.sage` + sage-vpn ssh config).
