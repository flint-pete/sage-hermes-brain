...
[
    {
        ...
        "handler": {
            "type": "image",
            "args": {
                "url": "rtsp://NODE_CONTROL_PLANE_IP0:554/stream1"
            }
        }
    },
    {
...
```
http://<IP>:<PORT>/cgi-bin/api.cgi?cmd=Snap&channel=0&rs=abc&user=sage&password=CAMERA_PASSWORD=640&height=480
```
...
curl -s -o /tmp/cam-snap.jpg \
  'http://CAMERA_IP:PORT/cgi-bin/api.cgi?cmd=Snap&channel=0&rs=test&user=sage&password=CAMERA_PASSWORD'
