...
```
(Node SSH is `USER@node-<VSN>.sage`, via the sage-vpn / `*.sage` ssh config —
...
```
SMOKING GUN: `dial tcp NODE_CONTROL_PLANE_IP:6443: i/o timeout` /
`http2: client connection lost` / `couldn't get current server API group list`.
`NODE_CONTROL_PLANE_IP:6443` is the in-cluster API-server VIP. If kubectl can't reach it:
...
```
ip route get NODE_CONTROL_PLANE_IP            # route exists but API still unreachable = link/cert, not routing
...
escalate: "node <VSN> lost its control-plane link to NODE_CONTROL_PLANE_IP:6443 after its
...
- Don't trust stale memory for token/host paths. The SES token has lived at
  `~/AI-projects/slack-hummingbird/secrets/sage-token.txt` (read with
  `read -r TOKEN < …`); node SSH is `USER@node-<VSN>.sage`. Verify paths exist
