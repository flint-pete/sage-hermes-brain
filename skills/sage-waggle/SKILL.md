...
triggers:
  ...
  - Querying sensor data from distributed nodes
...
1. **Edge Nodes** — ARM64/x86; WES/k3s. HONESTY: no fabrication; REAL names (`data-query-first-report-and-history.md`).`pywaggle2-producer-consumer-architecture`,`frame-anchored-batch-consumers-and-watchers`,`node-ssh`,`node-identity-and-upload-contract`,`pywaggle2-nodeinfo-gps-design`,`wes-pod-config-and-manifest-exposure`,`pluginctl-sideload-and-node-build`,`node-sideload-test-and-restore`,`audio-plugin-multinode-testing`,`storage-upload-health-verification`,`ci-handoff-doc-discipline`,`thor-arm64-deploy-pipeline`,`local-cache-ring-buffer`,`wes-node-service-daemonset-sideload`,`continuous-producer-patterns`,`plugin-design-doc-workflow`,`n
...
sudo pluginctl deploy -n my-counter NODE_CONTROL_PLANE_IP:5000/local/my-plugin  # Deploy (use descriptive names — visible across Sage)
...
- **External notifications (Slack, email, etc.)**: use a cloud-side watcher that polls the data API and reacts (node containers are network-restricted). Full pattern, webhook+image examples, secret mgmt, and reference impls: `references/cloud-trigger-notifications.md`.
- **Shareable web viz of fleet data** (public API + CORS proxy, themes, WebGL fallback): `references/sage-data-web-viz.md`.
